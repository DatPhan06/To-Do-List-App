  ...
  function initSortable() {
    new Sortable(todoList, {
      ...
      onUpdate: function (event) {
        event.item.style.transform = ''; // Reset transform after dragging is complete
        saveTodoOrder(); // Lưu thứ tự các công việc sau khi sắp xếp
      },
      onEnd: function (event) {
        event.from.querySelectorAll('.todo').forEach(todo => {
          todo.style.transform = ''; // Reset transform for all items in the original list
        });
      },
    });
  }
  
  // Hàm lưu thứ tự công việc sau khi sắp xếp vào localStorage
  function saveTodoOrder() {
    const todoItems = document.querySelectorAll('.todo');
    const todos = [];
    todoItems.forEach(todo => {
      todos.push(todo.querySelector('.todo-item').innerText);
    });
